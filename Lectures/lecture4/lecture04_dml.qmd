...
## Why orthogonality controls the error

Taylor-expand $E[\psi(W;\theta,\hat\eta)]$ around the truth $\eta_0$:

$$
E[\psi(W;\theta,\hat\eta)] \;\approx\; \underbrace{E[\psi(W;\theta,\eta_0)]}_{=\,0 \text{ at the truth}} \;+\; \underbrace{\partial_\eta E[\psi]\big|_{\eta_0}}_{=\,0 \text{ by orthogonality}}(\hat\eta-\eta_0) \;+\; \text{2nd-order terms}
$$

**The first-order term vanishes.** What survives is quadratic in $(\hat\eta-\eta_0)$ — a *product* of nuisance errors with themselves, not the error alone.

This is exactly where $O_p(n^{-1/4})\times O_p(n^{-1/4})=O_p(n^{-1/2})$ comes from, two slides ahead.

---

## Why the naive score fails: a concrete calculation

Same question, for a score that is **not** orthogonal: $\psi(W;\theta,\mu) = \mu_1(X)-\mu_0(X)-\theta$.
...
The naive score failed this test. Does AIPW pass it? Perturb $\mu_1$ in direction $\tilde\mu_1$ and compute the same derivative:
